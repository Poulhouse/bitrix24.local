...
<body>
<?
$file_path = $_SERVER['DOCUMENT_ROOT'] . "/local/pages/logs/ss/log.txt"; //путь к лог файлу с ошибками
$logs = file($file_path);
...
<? else : ?>
	...
	<table class="table table-bordered table-hover">
		...
		<?foreach ($result as $i => $item) : ?>
			...
			<tr class="<?=$classes;?>">
				<td><?=$i+1;?></td>
				<td width="15%"><?=$item['object'];?></td>
				<td><?=$item['entityTypeId'];?></td>
				<td><?=$item['itemId'];?></td>
				<td><?=$item['direction'];?></td>
				<td width="15%"><?=$item['dateTime'];?></td>
				<td><?=$item['jsonData'];?></td>
				<td><?=$item['response'];?></td>
				<td><?=$item['error'];?></td>
				<td><?=$item['link'];?></td>
