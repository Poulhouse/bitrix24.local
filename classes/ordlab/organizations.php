...
class Organizations {
	const URL = "https://api.ord-lab.ru/api/v2/organizations";
	...
	*/
	...
	public static function Update($elementId) {
		$timeData = Logs\TimeData::start();
		Loader::includeModule('iblock');
		$IBLOCK_ID = 176;
		$dataOrganization = [];
		...
		if ($item)
		{
			...
			if($itemOrgProp_id !== "")
			{
				...
				if (defined("ORD_TOKEN_KEY_TEST") && defined("ORD_TOKEN_KEY"))
				{
					...
					$jsonResponse = \KPLab\Curl::put_ord(ORD_TOKEN_KEY, $url, $jsonData, $objectData, $timeData, $point);
					...
					if (!empty($jsonResponse['success']))
					{
						...
						return "Обновление данных организации {$title} не прошло! Ошибка!";
					}
				}
			} else {
				\CRest ::call('crm.timeline.comment.add', [
	...
	public static function GetInfo($elementId) {
		...
		if ($item)
		{
			...
			$arSelect = ["*","PROPERTY_1008","PROPERTY_1009","PROPERTY_1010","PROPERTY_1011","PROPERTY_1012","PROPERTY_1013","PROPERTY_1014"];
	...
	public static function GetPlatforms($elementId) {
		/**
		 * "platforms": [
		 * {
		 * "externalId":"myPlatform",
	...
	public static function updatePlatforms($platforms) {
		/**
		 * "platforms": [
		 * {
		 * "externalId":"myPlatform",
		...
		Logs\File::AddMessage($platforms,"Входящийе данные",LOG_ORDLAB_ORG);
