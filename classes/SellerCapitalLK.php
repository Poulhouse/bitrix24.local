...
use KPLab\API\V2\Model\Service\SyncService;
...
class SellerCapitalLK
{
    ...
    public function setAccountState(): ?array
    {
        ...
        $this->CURLObjectData['METHOD'] = "POST";
        $this->CURLObjectData['partnerName'] = "SE";
        ...
        if(\Bitrix\Main\Loader::IncludeModule('kplab.api')) {
            ...
            $auth = new CustomAuthHttpClientDecorator($base, new AuthScheme(TOKEN_LK),"key");
            $httpClient = HttpClientFactory::build(baseClient: $auth);
            $SyncService = new SyncService($httpClient,"POST", $queryUrl);

            $meta['methodName'] = __FUNCTION__;
            $meta['controllerName'] = get_class($this);

            $meta['objectData'] = $this->CURLObjectData;

            $response = $SyncService->Execute($meta);
            /*$jsonResponse = \KPLab\Curl::postWithQueryUrl(
                TOKEN_LK,
                $queryUrl,
                $this->CURLObjectData,
            ...
            Logs\File::AddMessage("Не загружен модуль `kplab.api`",get_class($this), LOG_LK);
            return null;
        }
    }
