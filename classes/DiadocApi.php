...
class DiadocApi {
	...
	public function start($docSmartId)
	{
		...
		if($MessageId == "") {
			...
			$MessageToPost = new DiadocApi\MessageToPost($myOrgBoxId, $counteragentOrgBoxId, $itemId);
			$MessageToPost->Get();
			...
			if ($fileInfo)
			{
				...
				if (file_exists($filePath))
				{
					...
					$documentAttachment = new DiadocApi\DocumentAttachment("nonformalized", $base64File, false, $fileName, $fileId);
					$documentAttachment->setMetadataItem("FileName","", true);
					$document = $documentAttachment->get();
