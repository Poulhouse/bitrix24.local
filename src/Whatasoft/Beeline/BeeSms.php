...
namespace Whatasoft\Beeline;

class BeeSms 
{
  private $user;
  private $pass;
	private $hostname = 'beeline.amega-inform.ru';
	private $path='/sms_send/';
	private $proxy_data=false;
	
	private $post_data=array();			// данные передаваемые на сервер
	private $multipost=false;			// множественный запрос по умолчанию false
	
	public function __construct($user=false,$pass=false,$hostname=false,$proxy_data=false) {
	...
	function get_post_request($invars) {
		...
		if( $this->proxy_data ) {
			$pd = $this->proxy_data;
			if( !$pd["host"] || $pd["host"] == "" ) return 'Не указан адрес прокси-сервера';
			else $host = $pd["host"];
