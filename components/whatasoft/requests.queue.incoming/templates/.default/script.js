  ...
  window.app = new Vue({
    ...
    mounted: function(){
      ...
      BX.addCustomEvent("onPullEvent-voximplant", BX.delegate(function(command,params){
        ...
        if (command == 'showExternalCall') {
          let crm = params.CRM;
          let isFound = (crm.FOUND == 'Y' && crm.CONTACT.ID != 0);
          console.log(crm);
          console.log(isFound);
          if (!isFound){
             let action = 'create_contact_by_ring';
              let sendparams = {
                'phone': params.phoneNumber,
              };
              console.log(sendparams);
              SendApiRequest(action, sendparams).then((response) => {
              console.log(response);
                let contactId = response.contact_id;
                  $('#popup-window-content-im-phone-call-view .im-phone-call-buttons-container .im-phone-call-btn-red').click();
                  this.createCardByContactId(contactId);
              });
          }

          if (isFound) {
            let contactId = crm.CONTACT_DATA.ID;
            $('#popup-window-content-im-phone-call-view .im-phone-call-buttons-container .im-phone-call-btn-red').click();
            this.createCardByContactId(contactId);
          }
        }
        
        if (command == 'hideExternalCall' && this.deal_id) {
          setTimeout(function(){
            BX.rest.callMethod(
    ...
    methods: {
      ...
      toggleGroup: function(_group_id){
        ...
        if(ind !== -1){
          ...
          this.remove_operator_from_group(_group_id);
        }else{
          this.filter.group_ids.push(_group_id);
          this.add_operator_to_group(_group_id);
        }
      },
      createCardByContactId: function (contactId) {
        let action = 'create_card_by_contact_id';
        let params = {
          'contact_id': contactId,
          'group_ids': this.filter.group_ids,
        };
        SendApiRequest(action, params).then((response) => {
          this.dealLoad(response.deal_id);
        });
      },
      // Добавить оператора к группе
      add_operator_to_group: function(_group_id) {
        let action = 'add_operator_to_group';
        let params = {'group_id':_group_id};
        SendApiRequest(action, params).then((response) => {
          //console.log(response);
        });
      },
      // Удалить оператора из группы
      remove_operator_from_group: function(_group_id) {
        let action = 'remove_operator_from_group';
        let params = {'group_id':_group_id};
        SendApiRequest(action, params).then((response) => {
          //console.log(response);
        });
