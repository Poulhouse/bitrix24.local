...
class RequestsEditComponent extends CBitrixComponent{
  ...
  private function processRequest(){
    ...
    switch($action){
      case 'add_operator_to_group':
        $this->callLog('add_operator_to_group');
        ...
        $groupId = $_POST['group_id'] ?? false;
        $userGroups = !empty($this->queue_group_ids) ? $this->queue_group_ids : [];
        
        if (!$operatorId) {
          $result['message'] = 'Не установлен внутрений номер для текущего пользователя';
          return $result;
        }
        
        if (!in_array($groupId, $userGroups)) {
          $result['message'] = 'Пользователь не имеет прав на данную очередь';
          return $result;
        }
        
        try {
          $asteriskQueueId = $this->getIncomingAsteriskQueueIdFromQueue($groupId);
        } catch(\Exception $exception) {
          $result['message'] = $exception->getMessage();
          return $result;
        }
        
        $arAsteriskQueues = [$asteriskQueueId];
        $this->callQueueApi->addOperatorToAsteriskQueues($operatorId, $arAsteriskQueues);
        $result['success'] = true;
        break;
      case 'remove_operator_from_group':
      
        $this->callLog('remove_operator_from_group');
        ...
        $arAsteriskQueues = [$asteriskQueueId];
        $this->callQueueApi->removeOperatorFromAsteriskQueues($operatorId, $arAsteriskQueues);
        $result['success'] = true;
        break;
      case 'create_contact_by_ring':
        ...
        $arParams['NAME']="Входящий";
        $new_contact_id = $ct->Add($arParams, true, array('DISABLE_USER_FIELD_CHECK' => true));
        $result['success'] = true;
        $result['contact_id'] = $new_contact_id;

        break;
      case 'create_card_by_contact_id':
        $this->prepareUserQueueGroupIds();
        $contact_id = $_POST['contact_id'] ?? false;
        $group_ids = $_POST['group_ids'] ?? [];
        $queue_group_ids = array_intersect($this->queue_group_ids, $group_ids);

        // Получаем список очередей для текущего пользователя
        if (!count($queue_group_ids)) {
          $result['message'] = 'Не выбрана ни одна очередь';
          return $result;
        }
        
        $firstQueueGroupId = array_shift($queue_group_ids);
        
        try {
          $handler = new \Whatasoft\Incoming\CallHandler();
          $dealId = $handler->createDealFromContact($contact_id, $firstQueueGroupId);
        } catch(\Exception $exception) {
          $result['message'] = $exception->getMessage();
          return $result;
        }
      ...
      $deal_category_id = $arDeal['CATEGORY_ID'] ? $arDeal['CATEGORY_ID'] : 0;
       ...
       if (!isset($this->crmFieldManager)){
        ...
        switch($type){
          ...
          case 'call_back':
            ...
            $obj_date->setTime($time, 0, 0);
            $bdate = BDateTime::createFromPhp($obj_date);
            $queFilter = ['IBLOCK_ID' => 37, 'PROPERTY_TYPE' => 100,  'PROPERTY_CATEGORIES.ID' => intval($_POST['category_selected_id'])];

            $queues = CIBlockElement::GetList([], $queFilter)->Fetch();
      ///print_r($queues);
      //print_r($queFilter);die();
            if ($queues){
              $arDealFields['UF_CRM_QUEUE_GROUP'] = $queues['ID'];
            }
  ...
  {
    try {
      $reportTypeCode = \Whatasoft\Statistic\CallReport::REPORT_TYPE_INCOMING;
  ...
  private function getIncomingAsteriskQueueIdFromQueue($queueId)
  {
    $queueStorage = new \Whatasoft\Storage\IBlockElementStorage(IBLOCK_QUEUE_ID);
    $asteriskQueueStorage = new \Whatasoft\Storage\IBlockElementStorage(IBLOCK_INCOMING_ASTERISK_QUEUE_ID);
    ...
    $id = $arQueueProps['INCOMING_ASTERISK_QUEUE_ID']['VALUE'] ?? false;
    ...
    if (!$asteriskQueueId) {
      throw new \Exception("Входящая очередь Астериск не найдена");
    }
  ...
  private function prepareDealGroup(){
    $groupId = $this->arResult['DEAL']['UF_CRM_QUEUE_GROUP'];
    ...
    if($obQueueGroup = $dbQueueGroups->GetNextElement()){
      ...
      if($arQueueGroup['SUBMIT_INTERVAL'] <= 5){
        $arQueueGroup['SUBMIT_INTERVAL'] = QUEUE_DEAL_SUBMIT_INTERVAL_INCOMING;
      }
...
$orderedScriptSections = [];
  ...
  private function callLog($methodName)
  {
    ...
    dd_to_log($data, 'incoming_call_log.txt');
  }
