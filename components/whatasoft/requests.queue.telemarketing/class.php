...
class RequestsTelemarketingQueueComponent extends CBitrixComponent{
  const TELEMARKETING_ID = 102;
  ...
  private function prepareQueueGroups(){
    ...
    $arFilter['PROPERTY_TYPE'] = self::TELEMARKETING_ID;
    $arSelect = array(
      'ID',
      'IBLOCK_ID',
      'NAME',
  ...
  private function prepareDeals() {
    ...
    $queueStorage = new Whatasoft\Storage\HlBlockStorage(HLBLOCK_CAMPAIGIN_QUEUE_ID);
    $arFilter = [
      'UF_QUEUE_ID' => $this->queue_group_ids,
      'UF_ACTIVE' => 'Y',
      '!UF_IS_EXEC' => 'Y',
    ];
    $arQueueList = $queueStorage->getFiltered([], $arFilter)->fetchAll();
    $arDealQueueList = [];
    
    foreach($arQueueList as $arQueue) {
      $arDealQueueList[$arQueue['UF_DEAL_ID']] = $arQueue;
    }
    
    $dealIds = array_keys($arDealQueueList);
    $dealIds = count($dealIds) ? $dealIds : false;
    ...
    while($arDeal = $dbDeals->GetNext()){
      $id = $arDeal['ID'];
      $arDealQueue = $arDealQueueList[$id] ?? [];
      $arDeal['COLOR'] = '#33991a';
      $arDeal['EXPIRED'] = 'N';
      $group_id = $arDealQueue['UF_QUEUE_ID'];
      $arDeal['QUEUE_GROUP_ID'] = $group_id;
      $arDeal['CONTACT_PHONE'] = $arDealQueue['UF_PHONE'];
