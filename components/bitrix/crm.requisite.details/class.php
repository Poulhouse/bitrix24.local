...
class CCrmRequisiteDetailsComponent extends CBitrixComponent
{
	...
	protected function getBankDetailFieldsInfo()
	{
		if ($this->bankDetailFieldsInfo === null)
		{
            ...
            // 🛠️ Добавляем UF поля вручную
            foreach ($this->bankDetail->getFormUserFieldsInfo() as $fieldName => $fieldInfo)
            {
                $this->bankDetailFieldsInfo[$fieldName] = $fieldInfo;
            }
		}
    ...
    protected function prepareBankDetailsFields()
    {
        ...
        foreach ($this->rawBankDetailList as $pseudoId => $rawBankDetail) {
            if ($bankDetailFields === null) {
                // Получаем все поля, включая пользовательские
            ...
            foreach ($bankDetailFields as $fieldName => $fieldInfo) {
                $fieldValue = $rawBankDetail[$fieldName] ?? null;

                if ($fieldInfo['isUF']) {
                    // Добавьте userField в массив $fields:
                    ...
                    $fields[] = [
                        ...
                        'data' => [
                            'fieldInfo' => [
                                ...
                                'ENTITY_ID' => 'CRM_BANK_DETAIL',
                                'ENTITY_VALUE_ID' => $pseudoId,
                                'FIELD' => $fieldName,
	...
	protected function prepareBankDetailsData()
	{
        ...
        foreach ($this->rawBankDetailList as $pseudoId => $rawBankDetail) {
            ...
            foreach ($bankDetailFields as $fieldName => $fieldInfo) {
                $fieldValue = $rawBankDetail[$fieldName] ?? null;
                if (!empty($fieldInfo['isUF'])) {
                    if ($fieldInfo['type'] === 'boolean') {
                        $fieldValue = ($fieldValue === 'Y' || $fieldValue === 1 || $fieldValue === '1') ? 1 : 0;
                    } else {
                    ...
                    $fieldParams = [
                        'USER_TYPE_ID' => $fieldInfo['type'],
                        'ENTITY_ID' => $this->bankDetail->getUfId(),
                        'ENTITY_VALUE_ID' => $pseudoId,
                        'FIELD' => $fieldName,
                        'MULTIPLE' => $fieldInfo['multiple'] ?? 'N',
                        'MANDATORY' => $fieldInfo['required'] ?? 'N',
                        'SETTINGS' => $fieldInfo['settings'] ?? [],
                    ];

                    if (!$isEmptyField) {
                        $fieldParams['VALUE'] = is_array($fieldValue) ? array_values($fieldValue) : $fieldValue;
                    }
	...
	protected function prepareJsParams()
	{
        ...
        //$params['bankDetailsFieldsMap'][] = 'UF_CRM_PRIMARY_ACC';
