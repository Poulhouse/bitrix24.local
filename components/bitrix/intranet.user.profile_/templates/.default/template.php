...
<div class="intranet-user-profile-column-block">
	...
	<span class="intranet-user-profile-column-block-title-text">ГК СОДЕЙСТВИЕ</span>
	</div>
<p><b>Офис: </b><? echo ss_office($arResult["User"]['ID']); ?></p>

<?
if (!empty(ss_contact($arResult["User"]['ID'])))
	{ ?>	
	<p><b>Контакт: </b><? echo ss_contact($arResult["User"]['ID']); ?></p>
	<?
	}
	?>
<?
if (!empty(ss_agent($arResult["User"]['ID'])))
	{ ?>	
	<p><b>Агент: </b><? echo ss_agent($arResult["User"]['ID']); ?></p>
	<?
	}
	?>
</div>
...
<div class="intranet-user-profile-column-block">
	<div class="intranet-user-profile-column-block-title">
		<span class="intranet-user-profile-column-block-title-text">
			Содкоины
		</span>
	</div>				
	<table border="0" cellpadding="0" cellspacing="0" width="100%">
		<tr border="0" cellpadding="0" cellspacing="0">
			<td width="10"></td>
			<td width="30">
			<img border="0" src="https://crm.sodeistvie.su/services_sodeistvie/images/S.png" width="30" height="30"></td>
			<td width="10"></td>
			<td>
			<font size="4"><? echo ss_sodcoinCount($arResult["User"]['ID']); ?></font>
			...
			<td>
			<p align="right"><? if ($USER->getId() != $arResult["User"]["ID"])
			{ 
			?>
			<a href="javascript:void(0);" onclick="BX.SidePanel.Instance.open('https://crm.sodeistvie.su/services_sodeistvie/sodkoin/in.php', {
			requestMethod: 'post',
			requestParams: {
							userid: '<?=intval($arResult["User"]["ID"])?>',
							username: '<?=$arResult["User"]["NAME"]." ".$arResult["User"]["LAST_NAME"]?>',//TODO
							operation: 'rashod',//TODO
							},
			width: 300
			}); return event.preventDefault();" class="ui-btn ui-btn-xs ui-btn-light-border ui-btn-round">Наградить</a>	
			<?
			} else {echo 'Потратить';}?>
