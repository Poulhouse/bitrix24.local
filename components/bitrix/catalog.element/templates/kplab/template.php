			...
			<div class="col-md-12 col-sm-12">
				...
				<dl class="row">
					...
					foreach ($arResult['DISPLAY_PROPERTIES'] as $property)
					{
						...
						<dd class="col-sm-9" style="margin-bottom: 15px;">
							<?
							if($property['ID'] == 964){
								$json964 = strip_tags(htmlspecialchars_decode($property['DISPLAY_VALUE']));
								echo "<div id='codeJson964' style='max-height: 410px;overflow-y: auto;background-color: ivory;color: darkorange;'></div>";
								echo "
								<script>
									let my_json964 = ".$json964.";
									let result964 = JSON.stringify(my_json964, null, 4);
									document.getElementById('codeJson964').innerHTML = '<pre>' + result964 + '</pre>';
								</script>
								";
							} elseif($property['ID'] == 963){
								$json963 = strip_tags(htmlspecialchars_decode($property['DISPLAY_VALUE']));
								echo "<div id='codeJson963' style='max-height: 250px;overflow-y: auto;background-color: ivory;color: darkorange;'></div>";
								echo "
								<script>
									let my_json963 = ".$json963.";
									let result963 = JSON.stringify(my_json963, null, 4);
									document.getElementById('codeJson963').innerHTML = '<pre>' + result963 + '</pre>';
								</script>
								";
							}
							elseif($property['ID'] == 969){
								$json969 = strip_tags(htmlspecialchars_decode($property['DISPLAY_VALUE']));
								if(isJson($json969))
								{
									echo "<div id='codeJson969' style='max-height: 250px;overflow-y: auto;background-color: ivory;color: darkorange;'></div>";
									echo "
									<script>
										let my_json969 = " . $json969 . ";
										let result969 = JSON.stringify(my_json969, null, 4);
										document.getElementById('codeJson969').innerHTML = '<pre>' + result969 + '</pre>';
									</script>
									";
								} else {
									echo (is_array($property['DISPLAY_VALUE']) ? implode(' / ', $property['DISPLAY_VALUE']) : $property['DISPLAY_VALUE']);
								}
							}
							else {
							?>
