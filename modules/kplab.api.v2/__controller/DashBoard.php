...
class DashBoard extends \Bitrix\Main\Engine\Controller
{
    ...
    public function getDealsForPartnerAction(array $params = [])
    {
        ...
        $strDealSQL = $startDate === null
            ? "SELECT * FROM b_crm_deal INNER JOIN b_uts_crm_deal ON b_crm_deal.ID = b_uts_crm_deal.VALUE_ID WHERE b_crm_deal.CATEGORY_ID = '22' AND DATE(b_crm_deal.DATE_MODIFY) <= '{$endDate}'  ORDER BY b_crm_deal.ID ASC LIMIT " . $qty . " OFFSET " . $offset . ";"
            : "SELECT * FROM b_crm_deal INNER JOIN b_uts_crm_deal ON b_crm_deal.ID = b_uts_crm_deal.VALUE_ID WHERE b_crm_deal.CATEGORY_ID = '22' AND (b_crm_deal.DATE_MODIFY BETWEEN '{$startDate}' AND '{$endDate}') ORDER BY b_crm_deal.ID ASC LIMIT " . $qty . " OFFSET " . $offset . ";";

        $resCountItemsQuery = $DB->query($strCountDealsSQL);
        $totalItems = $resCountItemsQuery->Fetch()['COUNT(*)'];

        //Logs\File ::AddMessage($totalItems, "totalItems", LOG_API_SYNC_DEAL_LOANSELLER_CONTROLLER);

        // Получение элементов
        $arItems['object'] = (string) "deals";
        ...
        while ($resItem = $resItemsQuery->Fetch()) {
            ...
            $res = $this->prepareDataArray($resItem, $fieldsMetadata, $excludedTitles, $includedTitles, $uf);
            $arItems['results'][] = $res;
        }
    ...
    public function getUnderwritingItemsAction(array $params = [])
    {
        ...
        while ($resUnderwritingItem = $resUnderwritingItemsQuery->Fetch()) {
            ...
            $res = $this->prepareDataArray($resUnderwritingItem, $fieldsMetadata, $excludedTitles);
    ...
    public function getOSKItemsAction(array $params = [])
    {
        ...
        while ($resItem = $resItemsQuery->Fetch()) {
            ...
            $res = $this->prepareDataArray($resItem, $fieldsMetadata, $excludedTitles);
            $arItems['results'][] = $res;
        }
    ...
    public function getPartnerSupportItemsAction(array $params = [])
    {
        ...
        while ($resFincontrolItem = $resFincontrolItemsQuery->Fetch()) {
            $excludedTitles = [];
            $res = $this->prepareDataArray($resFincontrolItem, $fieldsMetadata, $excludedTitles);
    ...
    public function getDealsForSalesAction(array $params = [])
    {
        ...
        $strDealSQL = $startDate === null
            ? "SELECT * FROM b_crm_deal INNER JOIN b_uts_crm_deal ON b_crm_deal.ID = b_uts_crm_deal.VALUE_ID WHERE b_crm_deal.CATEGORY_ID = '23' AND DATE(b_crm_deal.DATE_MODIFY) <= '{$endDate}'  ORDER BY b_crm_deal.ID ASC LIMIT " . $qty . " OFFSET " . $offset . ";"
            : "SELECT * FROM b_crm_deal INNER JOIN b_uts_crm_deal ON b_crm_deal.ID = b_uts_crm_deal.VALUE_ID WHERE b_crm_deal.CATEGORY_ID = '23' AND (b_crm_deal.DATE_MODIFY BETWEEN '{$startDate}' AND '{$endDate}') ORDER BY b_crm_deal.ID ASC LIMIT " . $qty . " OFFSET " . $offset . ";";
        ...
        $resItemsQuery = $DB->query($strDealSQL);
        $fieldsMetadata = $this->getFieldsMetadata('CRM_DEAL', \CCrmOwnerType::Deal);
        //$fieldsInfo = \Bitrix\Crm\Service\Container::getInstance()->getFactory(\CCrmOwnerType::Deal)->getFieldsInfo();
    ...
    public function getLeadsAction(array $params = [])
    {
        ...
        $strLeadsSQL = $startDate === null
            ? "SELECT * FROM b_crm_lead INNER JOIN b_uts_crm_lead ON b_crm_lead.ID = b_uts_crm_lead.VALUE_ID WHERE DATE(b_crm_lead.DATE_MODIFY) <= '{$endDate}'  ORDER BY b_crm_lead.ID ASC LIMIT " . $qty . " OFFSET " . $offset . ";"
            : "SELECT * FROM b_crm_lead INNER JOIN b_uts_crm_lead ON b_crm_lead.ID = b_uts_crm_lead.VALUE_ID WHERE (b_crm_lead.DATE_MODIFY BETWEEN '{$startDate}' AND '{$endDate}') ORDER BY b_crm_lead.ID ASC LIMIT " . $qty . " OFFSET " . $offset . ";";

        $resCountItemsQuery = $DB->query($strCountLeadsSQL);
        $totalItems = $resCountItemsQuery->Fetch()['COUNT(*)'];

        Logs\File ::AddMessage($totalItems, "totalItems", LOG_API_SYNC_LEADS_CONTROLLER);

        // Получение элементов
        $arItems['object'] = (string) "leads";
        ...
        if ($enumFieldID !== null) {
            switch ($enumField) {
                case 'SOURCE_ID':
                    ...
                    break;
                case 'STAGE_ID':
                    $strStatusSQL = "SELECT * FROM b_crm_status WHERE ENTITY_ID LIKE '%DYNAMIC_%_STAGE%' AND STATUS_ID='{$enumFieldID}';";
                    break;
    ...
    private function prepareDataArray($item, $fieldsMetadata, $excludedTitles, $includedTitles = [], $uf = false): array
    {
        $result = [];
        if($uf) {
            ...
            /*foreach ($fieldsMetadata as $bitrixFieldName => $metadata) {
                $originalTitle = $metadata['originalTitle'];
                $title = $metadata['title'];


                /*if(!empty($includedTitles) && !in_array($originalTitle, $includedTitles)) {
                    continue; // Пропускаем обработку если нет в списке
                }

                // Проверяем, является ли поле исключенным
                if (in_array($originalTitle, $excludedTitles)) {
                    continue; // Пропускаем обработку этого поля
                }*/

            // Обработка полей на основе их типов
            /*switch ($metadata['type']) {
                case 'string':
                    $result[$title] = isset($item[$bitrixFieldName]) ? (string) $item[$bitrixFieldName] : null;
                    break;
                case 'datetime':
                    $result[$title] = isset($item[$bitrixFieldName]) ? date('Y-m-d\TH:i:s.msp', strtotime($item[$bitrixFieldName])) : null;
                    break;
                case 'date':
                    $result[$title] = isset($item[$bitrixFieldName]) ? date('Y-m-d\TH:i:s.msp', strtotime($item[$bitrixFieldName])) : null;
                    break;
                case 'integer':
                    $result[$title] = isset($item[$bitrixFieldName]) ? (int) $item[$bitrixFieldName] : null;
                    break;
                case 'double':
                    $result[$title] = isset($item[$bitrixFieldName]) ? (float) $item[$bitrixFieldName] : null;
                    break;
                case 'boolean':
                    $result[$title] = isset($item[$bitrixFieldName]) ? (bool) $item[$bitrixFieldName] : false;
                    break;
                case 'enumeration':
                    $result[$title] = isset($item[$bitrixFieldName]) ? $this->getUserFieldEnumResult($item[$bitrixFieldName]) : null;
                    break;
                case 'crm':
                    $result[$title] = isset($item[$bitrixFieldName]) ? (int) $item[$bitrixFieldName] : null;
                    break;
                case 'user':
                    $result[$title] = isset($item[$bitrixFieldName]) ? $this->getUser($item[$bitrixFieldName]) : null;
                    break;
                case 'crm_status':
                    $result[$title] = isset($item[$bitrixFieldName]) ? $this->getStatusResult($item[$bitrixFieldName], $bitrixFieldName) : null;
                    break;
                case 'employee':
                    $result[$title] = isset($item[$bitrixFieldName]) ? $this->getUser($item[$bitrixFieldName]) : null;
                    break;
                case 'crm_category':
                    $result[$title] = isset($item[$bitrixFieldName]) ? $this->getCategory($item[$bitrixFieldName]) : null;
                    break;
                case 'file':
                    break;
                // Добавьте обработку других типов полей, если необходимо
