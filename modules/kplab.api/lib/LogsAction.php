...
class LogsAction
{
    public static function Request($objectData, $methodName, $url, $controllerName,
                                   ...
                                   $requestTypeId = 0, $outRequest, $partnerName = "Битрикс24", bool $dev = false): void
    {
        // Получаем настройки
        ...
        if (!$outRequest) {
            ...
            if(empty($partnerName)) {
                $partnerName = self::getPartnerName($authorization, $dev);
    ...
    public static function getPartnerName($authorization, bool $dev = false) {
        if ($dev) {
