...
class CompanyService
{
    ...
    private function buildPayload(array $data, string $externalSystem): array
    {
        ...
        /*if ($externalSystem == 'onec') {
            if ($orgStructure == "FL") {
                $syncObjects = [];
                $syncObject['typeId'] = 1;
                $syncObject['id'] = $data['UF_CRM_COMPANY_SS_AM_ID'] ?? "";

                $syncObject['lastName'] = $requisite[0]['RQ_LAST_NAME']; //Фамилия
                $syncObject['firstName'] = $requisite[0]['RQ_FIRST_NAME']; //Имя
                $syncObject['middleName'] = $requisite[0]['RQ_SECOND_NAME'] ?? ""; //Отчество
                $syncObject['birthDate'] = date('d.m.Y', strtotime($requisite[0]['UF_CRM_1684493639'])); //Дата рождения
                $syncObject['birthPlace'] = $requisite[0]['UF_CRM_1647929611']; //Место рождения
                $syncObject['taxNum'] = $requisite[0]['RQ_INN']; //ИНН
                $syncObject['socialNum'] = $requisite[0]['UF_CRM_1684476607']; //СНИЛС
                $syncObject['regMark'] = 0; //Признак индивидуального предпринимателя
                $syncObject['regNum'] = ""; //ОГРН
                $syncObject['docType'] = ($requisite[0]['RQ_IDENT_DOC'] == 'Паспорт гражданина Российской Федерации') ? 21 : 31; //Вид документа, удостоверяющего личность
                $syncObject['docSeries'] = $requisite[0]['RQ_IDENT_DOC_SER']; //Серия документа, удостоверяющего личность
                $syncObject['docNumber'] = $requisite[0]['RQ_IDENT_DOC_NUM']; //Номер документа, удостоверяющего личность
                $syncObject['docIssueDate'] = date('d.m.Y', strtotime($requisite[0]['RQ_IDENT_DOC_DATE'])); //Дата выдачи документа, удостоверяющего личность
                $syncObject['docIssuerText'] = $requisite[0]['RQ_IDENT_DOC_ISSUED_BY']; //Место выдачи документа, удостоверяющего личность
                $syncObject['docDeptCode'] = $requisite[0]['RQ_IDENT_DOC_DEP_CODE']; //Код подразделения места выдачи документа, удостоверяющего личность

                //region Адреса
                $resAddrList = \B24Rest::call('crm.address.list', array(
                    'filter' => array('ANCHOR_ID' => $data['ID'], 'ANCHOR_TYPE_ID' => \CCrmOwnerType::Company),
                ...
                $syncObject['addressDetails'] = $addressDetails;
                //endregion
            ...
            if ($orgStructure == "IP") {
                ...
                $syncObject['id'] = $data['UF_CRM_COMPANY_SS_AM_ID'] ?? "";
                //region Реквизиты
                $resRequisite = \B24Rest::call(
                    "crm.requisite.list",
                    array(
                        "filter" => ["ENTITY_TYPE_ID" => \CCrmOwnerType::Company, "ENTITY_ID" => $data['ID']],
                        "select" => ['*',"UF_*"]
                    )
                )['result'];

                foreach ($resRequisite as $requisite) {
                    $syncObject['lastName'] = $requisite['RQ_LAST_NAME']; //Фамилия
                    $syncObject['firstName'] = $requisite['RQ_FIRST_NAME']; //Имя
                    $syncObject['middleName'] = $requisite['RQ_SECOND_NAME'] ?? ""; //Отчество
                    $syncObject['birthDate'] = date('d.m.Y', strtotime($requisite['UF_CRM_1684493639'])); //Дата рождения
                    $syncObject['birthPlace'] = $requisite['UF_CRM_1647929611']; //Место рождения
                    $syncObject['taxNum'] = $requisite['RQ_INN']; //ИНН
                    $syncObject['socialNum'] = $requisite['UF_CRM_1684476607']; //СНИЛС
                    $syncObject['regMark'] = 1; //Признак индивидуального предпринимателя
                    $syncObject['regNum'] = $requisite['RQ_OGRNIP']; //ОГРН
                    $syncObject['docType'] = ($requisite['RQ_IDENT_DOC'] == 'Паспорт гражданина Российской Федерации') ? 21 : 31; //Вид документа, удостоверяющего личность
                    $syncObject['docSeries'] = $requisite['RQ_IDENT_DOC_SER']; //Серия документа, удостоверяющего личность
                    $syncObject['docNumber'] = $requisite['RQ_IDENT_DOC_NUM']; //Номер документа, удостоверяющего личность
                    $syncObject['docIssueDate'] = date('d.m.Y', strtotime($requisite['RQ_IDENT_DOC_DATE'])); //Дата выдачи документа, удостоверяющего личность
                    $syncObject['docIssuerText'] = $requisite['RQ_IDENT_DOC_ISSUED_BY']; //Место выдачи документа, удостоверяющего личность
                    $syncObject['docDeptCode'] = $requisite['RQ_IDENT_DOC_DEP_CODE']; //Код подразделения места выдачи документа, удостоверяющего личность
                }
                //endregion
                //region Адреса
                $resAddrList = \B24Rest::call('crm.address.list', array(
        ...
        if ($org === self::TYPE_ORG) {
            ...
            if(!is_null($representativeCrmId)) {
                ...
                if ($representative == "") {
                    ...
                    \B24Rest::call('crm.timeline.comment.add', [
                        'fields' => [
    ...
    private function loadContactDetails(int $companyId): array
    {
        ...
        {
            if($multifaceted['VALUE_TYPE'] == 'MOBILE') {
                if($multifaceted['TYPE_ID'] == 'PHONE') $contactDetails[$i]['typeId'] = 1;
                if($multifaceted['TYPE_ID'] == 'EMAIL') $contactDetails[$i]['typeId'] = 2;
                $contactDetails[$i]['valueType'] = 1;
                $contactDetails[$i]['valueText'] = $multifaceted['VALUE'];
                $contactDetails[$i]['commentText'] = $multifaceted['ENTITY_ID'].'_'.$multifaceted['ELEMENT_ID'];
            }
            if($multifaceted['VALUE_TYPE'] == 'WORK') {
                if($multifaceted['TYPE_ID'] == 'PHONE') $contactDetails[$i]['typeId'] = 1;
                if($multifaceted['TYPE_ID'] == 'EMAIL') $contactDetails[$i]['typeId'] = 2;
                $contactDetails[$i]['valueType'] = 2;
                $contactDetails[$i]['valueText'] = $multifaceted['VALUE'];
                $contactDetails[$i]['commentText'] = $multifaceted['ENTITY_ID'].'_'.$multifaceted['ELEMENT_ID'];
            }
            if($multifaceted['VALUE_TYPE'] == 'HOME') {
                if($multifaceted['TYPE_ID'] == 'PHONE') $contactDetails[$i]['typeId'] = 1;
                if($multifaceted['TYPE_ID'] == 'EMAIL') $contactDetails[$i]['typeId'] = 2;
                $contactDetails[$i]['valueType'] = 3;
                $contactDetails[$i]['valueText'] = $multifaceted['VALUE'];
                $contactDetails[$i]['commentText'] = $multifaceted['ENTITY_ID'].'_'.$multifaceted['ELEMENT_ID'];
            }
