...
class Leads extends \Bitrix\Main\Engine\Controller
{
	...
	public function getLeadsByPartnerInnAction(array $params = [])
	{
		$timeData = Logs\TimeData::start();
		$context = Application ::getInstance() -> getContext();
		$request = $context -> getRequest();
	...
	public function setLeadsAction(array $params = []) {
		...
		if(empty($requestArray['leads'])) {
			...
			while($ob = $res->GetNextElement())
			{
				...
				self ::addLeadToStage($this, $leadStage, $leadSource, $requestArray);
			}
			/*if(!empty($requestArray['partnerInn']))
			{
				self ::addLeadToStage($this, "26", $requestArray['leads'], $requestArray['partnerInn']);
			}*/
			return true;
		}
	}
