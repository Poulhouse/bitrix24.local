...
define("LOG__ACTIVITY", $_SERVER['DOCUMENT_ROOT']."/local/activities/log.log");
 ...
 */
class CBPKPLabCrmGetDynamicInfoActivity extends \Bitrix\Bizproc\Activity\BaseActivity
{
	...
	protected function prepareProperties(): void
	{
		...
		try
		{
			$this->preparedProperties['DynamicEntityFields'] = $this->findEntityIds();
		}
		catch (\Bitrix\Main\ArgumentException $exception)
		{
			$this->preparedProperties['DynamicEntityFields'] = [];
		}
	}
    ...
    protected function findEntityIds(): array
    {
        $dynamicTypeId = $this->DynamicTypeId;
        if (!$dynamicTypeId)
        {
        ...
        if (isset($factory)) {
            ...
            //$instance = new self($this->name);
            // Получаем элементы без ограничения по количеству
            $items = $factory->getItems([
                'select' => ['ID'],  // Получаем только ID
                'filter' => $this->getOrmFilter($conditionGroup, $targetDocumentType),
            ]);
        }

        // Собираем ID всех найденных элементов
        $ids = [];
        foreach ($items as $item) {
            $ids[] = $item->getId();
        }

        return $ids;  // Возвращаем массив всех ID
    }
    ...
    {
        $simpleMap = $fieldsMap;
        unset($simpleMap['DynamicFilterFields']);  // Убираем фильтр из простых свойств
        ...
        if ($result->isSuccess())
        {
            ...
            // Извлекаем фильтр для динамических сущностей
            $currentValues['DynamicFilterFields'] = static::extractFilterFromProperties($dialog, $fieldsMap)->getData();

            // Если переменная DynamicEntityFields не существует, инициализируем её как массив
            if (!isset($currentValues['DynamicEntityFields']) || !is_array($currentValues['DynamicEntityFields'])) {
                $currentValues['DynamicEntityFields'] = [];
            }

            // Создаем экземпляр текущего класса
            $instance = new self($currentValues['Name']);  // Передаем необходимый параметр в конструктор, если нужно

            // Получаем все ID сущностей на основе типа и фильтра через экземпляр
            $entityIds = $instance->findEntityIds();

            // Если массив ID существует и это массив
            if (!empty($entityIds) && is_array($entityIds))
            {
                // Сохраняем все ID в DynamicEntityFields
                $currentValues['DynamicEntityFields'] = $entityIds;
            }
